...
namespace AmsiSandbox.GroupPolicyCom
{
    ...
    public abstract class GroupPolicyObjectBase
    {
        ...
        internal GroupPolicyObjectBase()
        {
        ...
        protected static T withSingleThreadedApartmentCheck<T>(Func<T> operation)
        {
            ...
            catch (InvalidCastException e)
            {
                ...
                {
                    throw new ApplicationException("Must require single threaded apartment.",  e);
                }
                else
                {
                    throw e;
        ...
        protected static void trycatch(Func<uint> operation, string messageTemplate, params object[] messageArgs)
        {
            ...
            if (result != S_OK)
            {
                ...
                throw new ApplicationException(string.Format("{0}. Error code {1} (see WinError.h)", message, result));
            }
        }
